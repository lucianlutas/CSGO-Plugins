...
public void OnPluginStart()
{
	...
	RegConsoleCmd("sm_ore", DOMenu);
	RegConsoleCmd("sm_totalore", Command_Wasted);
...
public int ShowTotalCallback(Handle owner, Handle hndl, char [] error, any client)
{
	...
	if(SQL_HasResultSet(hndl))
	{
		while (SQL_FetchRow(hndl))
		{
			...
			Format(textbuffer,128, "%s - %d h %d m %d s",name,g_iHours, g_iMinutes, g_iSeconds); 
			menu2.AddItem(steamid, textbuffer);
		}
	}
	if(order < 1) 
	{
		menu2.AddItem("empty", "TOP-ul e gol!");
	}
...
int ShowTimer(int Time, char[] buffer,int sizef)
{
	...
	if(g_iHours >= 1)
	{
		Format(buffer, sizef, "%d ore %d minute %d secunde", g_iHours, g_iMinutes, g_iSeconds );
	}
	else if(g_iMinutes >= 1)
	{
		Format(buffer, sizef, "%d minute %d secunde", g_iMinutes, g_iSeconds );
	}
	else
	{
		Format(buffer, sizef, "%d secunde", g_iSeconds );
	}
}
...
public Action DOMenu(int client, int args)
{
	...
	if(args > 0)
	{
		...
		SQL_TQuery(g_hDB, SQLShowPlayerTime, buffer, GetClientUserId(client));
		//LogToFileEx(g_sCmdLogPath, "Query %s", buffer);
	}
	else
	{
		Menu menu = CreateMenu(DIDMenuHandler);
		menu.SetTitle("[ORE] - Europa.1tap.ro");
		menu.AddItem("option1", "Vezi timpul tau");
		menu.AddItem("option2", "Top jucatori");
...
public int SQLShowPlayTime(Handle owner, Handle hndl, char [] error, any data)
{
	...
	if(!SQL_GetRowCount(hndl) || !SQL_FetchRow(hndl)) 
	{
		PrintToChat(client, " \x03SteamID-ul nu a fost gasit in baza de date.");
		return;
	}
	char name[124];
	...
	menu.SetTitle("Timpul jucat de tine");
	...
	Format(buffer, 124, "Total: %s", buffer);
	menu.AddItem("", buffer, ITEMDRAW_DISABLED);
...
public int DIDMenuHandler(Menu menu, MenuAction action, int client, int itemNum) 
{
	if( action == MenuAction_Select ) 
	{
		...
		if( strcmp(info,"option1") == 0 )
		{
			...
			menu2.SetTitle("Timpul jucat de tine");
...
public int SQLShowPlayerTime(Handle owner, Handle hndl, char [] error, any client)
{
	if(hndl == INVALID_HANDLE)
	{
		LogError("Query failure: %s", error);
	...
	while (SQL_FetchRow(hndl))
	{
		char buffer[124];
		ShowTimer(SQL_FetchInt(hndl, 0), buffer, sizeof(buffer));
		PrintToChatAll("[ORE] Timpul tau petrecut pe server este: %s", buffer);
	}
...
public int SQLShowWasteTime(Handle owner, Handle hndl, char [] error, any client)
{
	...
	while (SQL_FetchRow(hndl))
	{
		char buffer[124];
		ShowTimer(SQL_FetchInt(hndl, 0), buffer, sizeof(buffer));
		PrintToChatAll("[ORE] Timpul total al jucatoriilor pe server: %s", buffer);
	}
